...
function processPool(poolConfig) {
    ...
    config[engine] = localUtil.configurePool(poolConfig[engine], initialState[engine+'PoolDefault'] || {}, status)
    engines.getEngine(engine).poolConfig(config)
}
...
function processInterface( interfaceConfig ) {
    ...
    interfaceParams.parseReqObj = interfaceConfig.parseReqObj || initialState.parseReqObj
